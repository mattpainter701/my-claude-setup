...
### Step 3: Run Full Test or Verification Suite
Detect the project type before closing the sprint:

- Python: `py -m pytest --tb=short`
- Node.js/TypeScript: `npm test`
- Rust: `cargo test`
- Go: `go test ./...`
- KiCad/hardware projects: run the repo's documented DRC/ERC or review flow

If the repo already defines a canonical verification command in `.claude/CLAUDE.md`,
`CLAUDE.local.md`, `README.md`, `Makefile`, `package.json`, or `pyproject.toml`,
use that instead of guessing. If this setup is installed in the repo, prefer
`/verify full` because it already handles project-type detection.
